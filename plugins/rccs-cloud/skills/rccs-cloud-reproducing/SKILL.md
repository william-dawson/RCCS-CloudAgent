...
1. **Write the minimal clean sequence of calls that produces the user's
   ...
   way (a partition that turned out to be busy, say); skip the rest.
...
4. **Run once with `mode="live"` from an empty `CACHE_DIR`.** Then copy
   that resulting cache directory to sit next to the notebook file, and
   tell the user to commit both together if they're versioning this.
5. **If a partition choice isn't given or obvious, check
   `get_resources`/`sinfo` for idle capacity and pick from that** — don't
   guess a default.

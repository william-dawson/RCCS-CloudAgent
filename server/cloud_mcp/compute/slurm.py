...
from ..models import Job, JobSpec, JobState, JobStatus, map_slurm_state
...
class SlurmBackend(SchedulerBackend):
    name = "slurm-rccs-cloud"

    def _header(self, spec: JobSpec) -> list[str]:
        res = spec.resources
        attr = spec.attributes
        # gpus → --gpus=<n>; not used for qc-gh200 / ng-dgx
        ...
        # R-CCS Cloud requires source /etc/profile for the module system.
        lines.extend(["", "source /etc/profile"])
        return lines
